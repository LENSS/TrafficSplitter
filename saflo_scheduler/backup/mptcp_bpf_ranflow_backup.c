...
struct ranflow_key_t{
	u32 token;
	s16 local_id;
	u8 remote_id;
};

struct ranflow_data_t{
	u32 avg_pacing;
	u32 wmem;
	u64 linger_time;
	bool enabled;
...
struct {
    ...
    __type(key, struct ranflow_key_t);
    __type(value, struct ranflow_data_t);
	__uint(pinning, LIBBPF_PIN_BY_NAME); //Not sure HMM
} ranflow_map SEC(".maps");
...
SEC("struct_ops")
void BPF_PROG(mptcp_sched_ranflow_init, struct mptcp_sock *msk)
{
}

SEC("struct_ops")
void BPF_PROG(mptcp_sched_ranflow_release, struct mptcp_sock *msk)
{
}

static int bpf_ranflow_get_send(struct mptcp_sock *msk,
			      struct mptcp_sched_data *data)
{
	...
	for (i = 0; i < data->subflows && i < MPTCP_SUBFLOWS_MAX; i++) {
		bool backup;
		struct ranflow_key_t key = {};
		struct ranflow_data_t *value, new_value = {};
		...
		value = bpf_map_lookup_elem(&ranflow_map, &key);
		if (!value) {
			...
			if (bpf_map_update_elem(&ranflow_map, &key, &new_value, BPF_ANY) != 0) {
		...
		if (linger_time < send_info[backup].linger_time && value->enabled == 1) {
...
static int bpf_ranflow_get_retrans(struct mptcp_sock *msk,
				 struct mptcp_sched_data *data)
{
...
SEC("struct_ops")
int BPF_PROG(bpf_ranflow_get_subflow, struct mptcp_sock *msk,
	     struct mptcp_sched_data *data)
{
	if (data->reinject)
		return bpf_ranflow_get_retrans(msk, data);
	return bpf_ranflow_get_send(msk, data);
}

SEC(".struct_ops")
struct mptcp_sched_ops ranflow = {
	.init		= (void *)mptcp_sched_ranflow_init,
	.release	= (void *)mptcp_sched_ranflow_release,
	.get_subflow	= (void *)bpf_ranflow_get_subflow,
	.name		= "bpf_ranflow",
};
