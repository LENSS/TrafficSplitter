			      ...
			      struct mptcp_sched_data *data)
{
	...
	for (i = 0; i < data->subflows && i < MPTCP_SUBFLOWS_MAX; i++) {
		...
		if (!value) {
			/* If the socket is just created, the eBPF scheduler randomly enables subflows using "rand" */
			unsigned int rand;
			...
			new_value.enabled = rand & 1;  // 0 or 1 with ~50% probability
			//bpf_printk("TEST: newly created: %d\n", new_value.enabled);
			new_value.safe = 1;
	...
	/* The below is the part making the final decision for subflow selection */
	/* If all subflows are disabled, it selects subflow 0. */
	if (send_info[SSK_MODE_ACTIVE].subflow_id == MPTCP_SUBFLOWS_MAX)
		subflow = bpf_mptcp_subflow_ctx_by_pos(data, 0);
	/* Otherwise, there must be selected subflow already. */
	else
