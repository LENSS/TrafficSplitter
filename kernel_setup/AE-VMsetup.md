...
This document describes the VM setup for the Artifact Evaluation (AE) process. We have already prepared and provided the VMs as OVA files, and this document serves as an additional supplementary guide.
...
2. Create a VirtualBox VM.
   - We recommend allocating at least **50 GB of storage, 3 CPU cores, and 8 GB of RAM**.
   - These resources are mainly required for the kernel compilation process.
   - After the kernel has been successfully compiled and installed, you can reduce the allocated resources if needed.
...
8. Compile and install the kernel. This step may take some time.
...
2. Move to the `src` directory and run the scripts to build BPF tools, MPTun proxy servers/clients, and MPTCP schedulers.
...
3. Check the resulting files.

   ```bash
   ls MPTun_proxy/build/ Saflo_scheduler/build/
   ```

4. Optionally, remove the `bpftool` files because `bpftool` is already installed.
